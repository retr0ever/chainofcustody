...
class TestMirnasForOffTargetCellType:
    ...
    def test_returns_dataframe(self, three_mirna_data):
        ...
        result = mirnas_for_off_target_cell_type("Liver", mature_seqs, df_expr, df_grouped)
    ...
    def test_returns_all_passing_mirnas(self, three_mirna_data):
        ...
        mature_seqs, df_expr, df_grouped = three_mirna_data
        result = mirnas_for_off_target_cell_type("Liver", mature_seqs, df_expr, df_grouped, threshold=0.0, top_n=10)
        assert len(result) == 3
    ...
    def test_threshold_filters_low_expression(self, three_mirna_data):
        ...
        mature_seqs, df_expr, df_grouped = three_mirna_data
        result = mirnas_for_off_target_cell_type("Liver", mature_seqs, df_expr, df_grouped, threshold=100.0)
    ...
    def test_threshold_exact_boundary_included(self, three_mirna_data):
        ...
        mature_seqs, df_expr, df_grouped = three_mirna_data
        result = mirnas_for_off_target_cell_type("Liver", mature_seqs, df_expr, df_grouped, threshold=50.0)
    ...
    def test_top_n_limits_result_size(self, three_mirna_data):
        mature_seqs, df_expr, df_grouped = three_mirna_data
        result = mirnas_for_off_target_cell_type("Liver", mature_seqs, df_expr, df_grouped, threshold=0.0, top_n=2)
        assert len(result) == 2

    def test_top_n_picks_lowest_entropy(self, three_mirna_data):
        """top_n=1 should return the miRNA with the lowest entropy."""
        mature_seqs, df_expr, df_grouped = three_mirna_data
        result = mirnas_for_off_target_cell_type("Liver", mature_seqs, df_expr, df_grouped, threshold=0.0, top_n=1)
    ...
    def test_invalid_off_target_cell_type_raises_value_error(self, three_mirna_data):
        ...
        with pytest.raises(ValueError, match="Kidney"):
            mirnas_for_off_target_cell_type("Kidney", mature_seqs, df_expr, df_grouped)
    ...
    def test_empty_result_when_nothing_passes_threshold(self, three_mirna_data):
        mature_seqs, df_expr, df_grouped = three_mirna_data
        result = mirnas_for_off_target_cell_type("Liver", mature_seqs, df_expr, df_grouped, threshold=9999.0)
        assert result.empty
    ...
    def test_mean_expr_values_match_off_target_cell_type_column(self, three_mirna_data):
        ...
        mature_seqs, df_expr, df_grouped = three_mirna_data
        result = mirnas_for_off_target_cell_type("Brain", mature_seqs, df_expr, df_grouped, threshold=0.0, top_n=10)
        for _, row in result.iterrows():
